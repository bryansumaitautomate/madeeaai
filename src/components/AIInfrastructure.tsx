...
const SchematicAnimation = () => {
  return (
    <div className="relative w-full h-[400px] flex items-center justify-center">
      ...
      <motion.div
        className="absolute w-60 h-60 rounded-full border border-primary/30"
        animate={{ rotate: -360 }}
      ...
      {/* Inner ring */}
      <motion.div
        className="absolute w-40 h-40 rounded-full border border-primary/40"
        animate={{ rotate: 360 }}
      ...
      {[0, 45, 90, 135, 180, 225, 270, 315].map((angle, i) => (
        <motion.div
          key={`line-${i}`}
          className="absolute h-px bg-gradient-to-r from-primary/50 to-transparent"
          style={{
            width: '160px',
            top: '50%',
            left: '50%',
      ...
      {/* Central core */}
      <motion.div
        className="relative w-24 h-24 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/40 flex items-center justify-center backdrop-blur-sm"
      ...
      {infrastructureLayers.map((layer, i) => {
        ...
        return (
          <motion.div
            key={layer.id}
            className="absolute flex items-center gap-2 px-3 py-1.5 rounded-full bg-card/80 border border-border backdrop-blur-sm"
            style={{
              left: `calc(50% + ${x}px)`,
              top: `calc(50% + ${y}px)`,
...
const AIInfrastructure = () => {
  return (
    <section className="relative py-32 overflow-hidden" style={{ backgroundColor: '#050505' }}>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-16">
          {infrastructureLayers.map((layer, index) => (
            <motion.div
              key={layer.id}
              className="group p-5 rounded-xl bg-card/30 border border-border"
              initial={{ opacity: 0, y: 20, scale: 1, borderColor: 'hsl(217 100% 55% / 0)', boxShadow: '0 0 0px hsl(217 100% 55% / 0)' }}
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl md:text-2xl font-serif text-foreground max-w-3xl mx-auto">
            ...
            <span className="italic text-primary">Infrastructure transforms revenue operations.</span>
          </p>
        </div>
        ...
        <div className="flex justify-center">
          <ControlRoomButton
            label="See Your Infrastructure Blueprint"
            icon={ArrowRight}
          />
        </div>
      </div>
